/**
 * This file has been claimed for ownership from @keycloakify/keycloak-account-ui version 260500.0.0.
...
export const PageNav = () => {
    ...
    usePromise(
        ...
        items => {
            const hiddenKeys = ["signingIn", "deviceActivity", "applications", "groups", "accountSecurity", "linkedAccounts"];

            const filterItems = (items: MenuItem[]): MenuItem[] => {
                return items
                    .filter(item => !hiddenKeys.includes(item.label))
                    .map(item => {
                        if ("children" in item) {
                            return {
                                ...item,
            ...
            setMenuItems(filterItems(items));
        }
    );
    return (
        <PageSidebar>
